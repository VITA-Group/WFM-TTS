...
def parse_args():
    ...
    parser.add_argument("--tts_mode", type=str, default="prob_beam-4", help="Type of test time scaling with sample number separated by '-', e.g., prob_beam-4, prm-2, orm-2")
    parser.add_argument(
        "--input_type",
        type=str,
...
def main(args):
    ...
    pipeline = ARVideo2WorldGenerationPipeline(
        ...
        test_time_scaling_mode=args.tts_mode
    )
    ...
    for idx, prompt_entry in enumerate(prompts_list):
        ...
        out_vid_list = pipeline.generate(
        ...
        if out_vid_list is None:
            log.critical("Guardrail blocked video2world generation.")
            continue

        # Save video
        for i, out_vid in enumerate(out_vid_list):
            if args.input_image_or_video_path:
                out_vid_path = os.path.join(args.video_save_folder, f"{args.video_save_name}_{i}.mp4")
            else:
                out_vid_path = os.path.join(args.video_save_folder, f"{os.path.basename(input_filename)[:-4]}_{i}.mp4")
            imageio.mimsave(out_vid_path, out_vid, fps=25)
            log.info(f"Saved video to {out_vid_path}")
        del inp_vid, out_vid_list
