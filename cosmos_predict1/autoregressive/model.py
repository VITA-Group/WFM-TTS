...
class AutoRegressiveModel(torch.nn.Module):
    ...
    def __init__(
        ...
        self.kv_cache = {} # save the kv cache before generation

        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        dino_model = torch.hub.load('facebookresearch/dino:main', 'dino_vitb16', source='github')
        self.dino = dino_model.to(self.device).eval()  
        ...
        # aes_path = f'checkpoints/aesthetic_model/emb_reader'
        # self.aesthetic_model = get_aesthetic_model(aes_path).to(self.device)
        self.aes_clip_model, _ = clip.load('ViT-L/14', device=self.device)
        self.aes_clip_model.eval()

    def temporal_consistency(self, decoded_images):
        dino_sim = 0.0
        clip_sim = 0.0
        ...
        dino_images = dino_transform(decoded_images.to(self.device))

        # CLIP preprocessing
        clip_images = self.clip_transform(decoded_images.to(self.device))
        clip_features = self.clip.encode_image(clip_images)
        clip_features = F.normalize(clip_features, dim=-1, p=2)

        for i in range(len(dino_images)):
            with torch.no_grad():
                # DINO similarity
                dino_image = dino_images[i].unsqueeze(0)
                dino_feat = self.dino(dino_image)
                dino_feat = F.normalize(dino_feat, dim=-1, p=2)

                # CLIP similarity
                clip_feat = clip_features[i].unsqueeze(0)

                if i == 0:
                    dino_first = dino_feat
                    clip_first = clip_feat
                else:
                    dino_sim_i = (max(0.0, F.cosine_similarity(dino_former, dino_feat).item()) +
                                max(0.0, F.cosine_similarity(dino_first, dino_feat).item())) / 2
                    clip_sim_i = (max(0.0, F.cosine_similarity(clip_former, clip_feat).item()) +
                                max(0.0, F.cosine_similarity(clip_first, clip_feat).item())) / 2
                    dino_sim += dino_sim_i
                    clip_sim += clip_sim_i
            dino_former = dino_feat
            clip_former = clip_feat

        n = len(dino_images) - 1
        return dino_sim / n + clip_sim / n
        
    def perceptual_quality(self, decoded_images):
        def _load_aesthetic_model():
            path = "checkpoints/aesthetic_model/emb_reader/sa_0_4_vit_l_14_linear.pth"
            if not os.path.exists(path):
                os.makedirs(os.path.dirname(path), exist_ok=True)
                url = ("https://github.com/LAION-AI/aesthetic-predictor/blob/main/"
                ...
                try:
                    ...
                    subprocess.run(['wget', url, '-P', os.path.dirname(path)])
            model = nn.Linear(768, 1)
            state_dict = torch.load(path)
            model.load_state_dict(state_dict)
            model.eval()
            return model.to(dtype=torch.float32, device=self.device)

        if not hasattr(self, "aesthetic_model"):
            self.aesthetic_model = _load_aesthetic_model()
        self.aesthetic_model = self.aesthetic_model.to(self.device).eval()

        image_batch = self.clip_transform(decoded_images).to(self.device)
        with torch.no_grad():
            feats = self.aes_clip_model.encode_image(image_batch).to(torch.float32)
            feats = F.normalize(feats, dim=-1, p=2)
            scores = self.aesthetic_model(feats).squeeze(dim=-1)
        return torch.mean(scores / 10, dim=0, keepdim=True)
    ...
    def generate(
        ...
        if tts_mode==None:
            ...
            return seq, None
        elif tts_mode.startswith("prm"):
            N = int(tts_mode.split("-")[1])
            spatial_token_num = np.prod(latent_shape[1:])
            path_steps = latent_shape[0] - prompt_len//spatial_token_num
            batch_size = next_token.shape[0]
            candidates = []
            if input_pos % spatial_token_num !=0:
                prompt_ph = 1
            else:
                prompt_ph = 0

            for step in range(path_steps):
                kv_cache_before_step = {}  # Store KV cache before decoding candidates

                # Save KV cache before decoding starts for fairness
                for layer_idx, layer in enumerate(self.model.layers):
                    kv_cache_before_step[layer_idx] = layer.attention.get_kv_cache()  # Ensure get_kv_cache() is implemented

                best_seq = None  # Best sequence found in this step
                best_score = float('-inf')

                # Store the original input_pos at the start of the step
                original_input_pos = input_pos.clone().detach()  # Clone to prevent modifications

                for i in range(N):
                    # Restore KV cache before each candidate decoding
                    for layer_idx, layer in enumerate(self.model.layers):
                        layer.attention.set_kv_cache(kv_cache_before_step[layer_idx])

                    # Restore input_pos to its original value before decoding
                    input_pos = original_input_pos.clone().detach()
                    ...
                    generated_tokens = decode_n_tokens(
                        ...
                        next_token.view(batch_size, -1),
                        input_pos.clone(), 
                        spatial_token_num - 1 if step==0 else spatial_token_num,  
                        temperature=temperature,
                        top_k=top_k,
                        top_p=top_p,
                        stop_tokens=stop_tokens,
                    ...
                    if verbose:
                        decode_time = time.time() - decode_start
                        prefill_throughput = seq.shape[1] / decode_time
                        decode_throughput = gen_len / decode_time
                        log.debug(f"[Decode] Time: {decode_time:.2f}s; Throughput: {decode_throughput:.2f} tokens/s")
                    ...
                    # Update `seq` with newly generated tokens
                    start_idx = prompt_len + 1
                    seq = seq[:, : start_idx + gen_len]
                    seq[:, start_idx:] = generated_tokens
                    
                    if not echo:
                        seq = seq[:, prompt_len:]

                    # Save KV cache for this candidate
                    for layer in self.model.layers:
                        layer.attention.save_kv_cache(i)
                    # Extract video from generated tokens
                    generation_tokens = seq[:, prompt_ph:]
                    indices_tensor = generation_tokens.long()

                    # Reshape the generated tokens into batch x time x height x width
                    ...
                    video_decoded = self.tokenizer.video_tokenizer.decode(indices_tensor.cuda(), t=indices_tensor.shape[1])

                    # Normalize decoded video from [-1, 1] to [0, 1], and clip values
                    video_decoded = (video_decoded * 0.5 + 0.5).clamp_(0, 1)
                    video_decoded = video_decoded.squeeze(0)

                    # Convert to uint8 for evaluation
                    video_decoded_eval = (video_decoded.permute(1, 0, 2, 3) * 255).clamp(0, 255).to(torch.uint8)

                    candidates.append(video_decoded_eval)

                    # Compute subject consistency score
                    temp_con = self.temporal_consistency(video_decoded_eval)
                    percep_qua = self.perceptual_quality(video_decoded_eval[-8:])
                    total_score = temp_con + percep_qua

                    # Track best sequence
                    if total_score > best_score:
                        best_score = total_score
                        best_seq = seq.clone()  # Clone to ensure data integrity
                        best_idx = i

                # Restore KV cache of the best sequence
                for layer in self.model.layers:
                    layer.attention.load_kv_cache(best_idx)
                gc.collect()
                torch.cuda.empty_cache()

                # Update `next_token` and `input_pos` for the next step
                next_token = best_seq[:, -1:].clone()
                input_pos = torch.tensor([best_seq.shape[1]-1], dtype=torch.long, device="cuda")
                prompt_len = best_seq.shape[1]-1
                empty = torch.empty(batch_size, total_len, dtype=prompt_tokens.dtype, device=prompt_tokens.device)
                empty[:, :prompt_len+1] = best_seq
                seq = empty
        ...
        elif tts_mode.startswith("prob_beam"):
            N = int(tts_mode.split("-")[1])
            top_k_select = int(np.sqrt(N))
            
            spatial_token_num = np.prod(latent_shape[1:])
            path_steps = latent_shape[0] - prompt_len//spatial_token_num

            T = 0.01
            k_n = int(N / top_k_select)
            spatial_token_num = 40 * 64
            ...
            candidate_pool = []  # Stores (score, sequence, best_idx)

            for step in range(path_steps):
                kv_cache_before_step = {}

                # Store KV cache before decoding starts for fairness
                if step==0:
                    for layer_idx, layer in enumerate(self.model.layers):
                        kv_cache_before_step[layer_idx] = layer.attention.get_kv_cache()

                candidates = []
                
                # Determine the number of trials for this step
                num_trials = N  # 4 in first step, then 2 per selected candidate
                original_input_pos = input_pos.clone().detach()  # Clone to prevent modifications

                if step > 1:
                    for trial in range(num_trials):
                        for layer in self.model.layers:
                            layer.attention.set_empty_kv_cache(trial, step-2)

                scores = []  # Store scores for probabilistic selection
                for trial in range(num_trials):
                    gc.collect()
                    torch.cuda.empty_cache()
                        
                    # If step > 0, restore KV cache from the corresponding best candidate
                    if step > 0:
                        best_idx = candidate_pool[trial // k_n][2]  # Get best_idx from last step's top candidates
                        best_seq = candidate_pool[trial // k_n][1]  # Get best_idx from last step's top candidates
                        for layer in self.model.layers:
                            layer.attention.load_kv_cache(best_idx, step-1)
                        next_token = best_seq[:, -1:].clone()
                        original_input_pos = torch.tensor([best_seq.shape[1]-1], dtype=torch.long, device="cuda")
                        prompt_len = best_seq.shape[1]-1
                        empty = torch.empty(batch_size, total_len, dtype=prompt_tokens.dtype, device=prompt_tokens.device)
                        empty[:, :prompt_len+1] = best_seq
                        seq = empty
                    ...
                    # Save KV cache for this candidate
                    for layer in self.model.layers:
                        layer.attention.save_kv_cache(trial, step)

                    # Decode video
                    generation_tokens = seq[:, prompt_ph:]
                    indices_tensor = generation_tokens.long()
                    indices_tensor = rearrange(indices_tensor, "B (T H W) -> B T H W", B=batch_size, H=latent_shape[1], W=latent_shape[2])
                    st_time = time.time()
                    video_decoded = self.tokenizer.video_tokenizer.decode(indices_tensor.cuda(), t=indices_tensor.shape[1])
                    video_decoded = (video_decoded * 0.5 + 0.5).clamp_(0, 1)
                    video_decoded = video_decoded.squeeze(0)
                    ...
                    candidates.append((total_score, seq.clone(), trial))
                    scores.append(total_score.item())  


                    torch.cuda.empty_cache()
                    del video_decoded_eval

                gc.collect()
                torch.cuda.empty_cache()
                # Convert scores to numpy and normalize for selection probability
                scores_ori = np.array(scores, dtype=np.float64)
                scores = scores_ori - np.min(scores_ori)  # Normalize to avoid tiny differences dominating
                probabilities = np.exp(scores / T) / np.sum(np.exp(scores / T))  # Softmax transformation

                # Select top_k_select candidates probabilistically
                selected_indices = np.random.choice(
                    num_trials, size=top_k_select, replace=False, p=probabilities
                )
                candidate_pool = [candidates[i] for i in selected_indices]  # Keep selected candidates

                # Free KV cache for non-selected candidates
                for trial in range(num_trials):
                    if trial not in selected_indices:
                        for layer in self.model.layers:
                            layer.attention.set_empty_kv_cache(trial, step)

            # Extract sequences from the top candidates
            final_candidate_list = [candidate[1] for candidate in candidate_pool]  # candidate[1] is best_seq
            final_candidate = torch.cat(final_candidate_list, dim=0)

            return final_candidate, None
        elif tts_mode.startswith("orm"):
            N = int(tts_mode.split("-")[1])
            spatial_token_num = np.prod(latent_shape[1:])
            batch_size = next_token.shape[0]
            if input_pos % spatial_token_num !=0:
            ...
            for i in range(N):
                ...
                # Compute subject consistency score
                temp_con = self.temporal_consistency(video_decoded_eval)
                percep_qua = self.perceptual_quality(video_decoded_eval)
                total_score = temp_con + percep_qua
