...
class Attention(nn.Module):
    ...
    def save_kv_cache(self, candidate_id, step_id=None):
        """Save KV cache for a specific candidate sequence."""
        if step_id:
            self.kv_cache[(step_id, candidate_id)] = {
                "cache_k": self.cache_k.clone(),
                "cache_v": self.cache_v.clone(),
            }
        else:
            self.kv_cache[candidate_id] = {
                "cache_k": self.cache_k.clone(),
                "cache_v": self.cache_v.clone(),
            }

    def load_kv_cache(self, candidate_id, step_id=None):
        """Load KV cache from a selected candidate."""
        if step_id:
            if (step_id, candidate_id) in self.kv_cache:
                self.cache_k = self.kv_cache[(step_id, candidate_id)]["cache_k"]
                self.cache_v = self.kv_cache[(step_id, candidate_id)]["cache_v"]
        else:
            if candidate_id in self.kv_cache:
                self.cache_k = self.kv_cache[candidate_id]["cache_k"]
                self.cache_v = self.kv_cache[candidate_id]["cache_v"]

    def set_empty_kv_cache(self, candidate_id, step_id=None):
        """Remove KV cache for a specific candidate sequence."""
        if step_id:
            self.kv_cache.pop((step_id, candidate_id), None)
        else:
            self.kv_cache.pop(candidate_id, None)

            
    def get_kv_cache(self):
        """Retrieve the current KV cache."""
        return {"cache_k": self.cache_k.clone(), "cache_v": self.cache_v.clone()}

    def set_kv_cache(self, cache_dict):
        """Set the KV cache from an external source."""
        if "cache_k" in cache_dict and "cache_v" in cache_dict:
            self.cache_k.copy_(cache_dict["cache_k"])
            self.cache_v.copy_(cache_dict["cache_v"])
        else:
            raise ValueError("KV cache dictionary must contain 'cache_k' and 'cache_v'.")
